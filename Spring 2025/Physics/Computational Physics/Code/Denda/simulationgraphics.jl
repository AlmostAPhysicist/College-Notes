...
# Record into a video file 

record(scene, "random_walk.mp4"; framerate = 60) do io

    global tip  # Ensure tip is recognized as a global variable

    # Update the scene at each frame
    for i in 1:100 #Initial frames
        sleep(1/100)
        recordframe!(io)
    end
    for i in 1:1_000 #Walk frames
        tip = tip .+ [rand()*rand((-1,1)) for i in 1:3] * 0.1
        arrow[] = [tip]
        push!(tracker[], tip)
        tracker[] = tracker[] #This updates the observable value
