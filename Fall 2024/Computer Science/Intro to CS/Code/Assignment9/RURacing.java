...
public class RURacing {
	...
	// WRITE YOUR CODE HERE
        ...
        int[] pos = moveRacer(0, 0, dist, map);
        String posString = "(" + pos[0] + "," + pos[1] + ")";
        StdOut.println("Racer1 is at: " + posString);
    }
    ...
    //The Premise/Assumption is that you START AT (0,0) and so you have to start moving
    //You would traverse through all n-1 OTHER ROAD sprites (not starting) and only land starting (the nth) sprite at the end

    int dist2MoveNext = 1;
    int distLeft = totalDist;
    ...
    while (flag) {
        ...
        if (distLeft > dist2MoveNext){
            pos = moveRacer(pos[0], pos[1], dist2MoveNext, map);
            String posString = "(" + pos[0] + "," + pos[1] + ")";
            StdOut.println("Racer2 is at: " + posString);

            distLeft -= dist2MoveNext; //This has now been traversed
            dist2MoveNext *= 2; //The next distance is now double for O(logn)
        }
        //So whenever the distLeft is less than or equal to the distance you have to move next, you just finish the race.
        else {
            pos = moveRacer(pos[0], pos[1], distLeft, map);
            String posString = "(" + pos[0] + "," + pos[1] + ")";
            StdOut.println("Racer2 is at: " + posString);
            flag = false;
        }
    ...
    while (flag) {
        ...
        if (distLeft > dist2MoveNext){
            pos = moveRacer(pos[0], pos[1], dist2MoveNext, map);
            String posString = "(" + pos[0] + "," + pos[1] + ")";
            StdOut.println("Racer3 is at: " + posString);

            distLeft -= dist2MoveNext; //This has now been traversed
            //This time the distance to move remains to be 1
            // dist2MoveNext *= 2; //The next distance is now double for O(logn)
        }
        //So whenever the distLeft is less than or equal to the distance you have to move next, you just finish the race.
        else {
    ...
    while (flag) {
        ...
        if (distLeft > dist2MoveNext){
            pos = moveRacer(pos[0], pos[1], dist2MoveNext, map);
            String posString = "(" + pos[0] + "," + pos[1] + ")";
            StdOut.println("Racer4 is at: " + posString);

            dist2MoveNext = 0;
            //For some reason they want to stay 1 less than total sprite counts (total - 1)
            //We already had one printed so total - 2
            for (int i = 0; i < totalDist-2; i++) {
                pos = moveRacer(pos[0], pos[1], dist2MoveNext, map);
                posString = "(" + pos[0] + "," + pos[1] + ")";
                StdOut.println("Racer4 is at: " + posString);
